defmodule JimmyWeb.BertNerLive do
  use JimmyWeb, :live_view

  # Taken from Guardian's homepage
  @default """
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, text: @default, answer: nil, running: false, task_ref: nil)}
  end
  ...
  def render(assigns) do
    ...
    <div class="h-screen w-screen flex items-center justify-center antialiased">
      <div class="flex flex-col h-1/2 w-1/2">
        <h1>BERT NER - named entity recognition</h1>
        <p>It analyses the given text and gives the list of named entities, which are also labeled (meaning it is able to distinguish between persons, locations, organizations).</p>
  ...
  def handle_event("predict", %{"text" => text}, socket) do
    task = Task.async(fn -> Nx.Serving.batched_run(Jimmy.BertNerServing, text) end)
    {:noreply, assign(socket, text: text, running: true, task_ref: task.ref)}
  end

  @impl true
  def handle_info({ref, %{entities: entities}}, %{assigns: %{task_ref: ref}} = socket) do
    answer =
      entities
      |> Enum.reduce(
        [],
        &["#{&1.phrase} (score #{Float.round(&1.score, 3)}, label: #{&1.label})" | &2]
      )
      |> Enum.reverse()
      |> Enum.join(", ")
