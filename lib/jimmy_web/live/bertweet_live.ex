defmodule JimmyWeb.BertweetLive do
  ...
  def render(assigns) do
    ...
    <div class="h-screen w-screen flex items-center justify-center antialiased">
      <div class="flex flex-col h-1/2 w-1/2">
        <h1>BERTweet - text emotion recognition</h1>
  ...
  def handle_event("predict", %{"text" => text}, socket) do
    task = Task.async(fn -> Nx.Serving.batched_run(Jimmy.BertweetServing, text) end)
    {:noreply, assign(socket, text: text, running: true, task_ref: task.ref)}
  end

  @impl true
  def handle_info({ref, %{predictions: predictions}}, %{assigns: %{task_ref: ref}} = socket) do
    label =
      predictions
      |> Enum.reduce([], &["#{&1.label} (score #{Float.round(&1.score, 3)})" | &2])
      |> Enum.reverse()
      |> Enum.join(", ")
